...
//! saves codebook parameters path
void BackgroundSubtractorSuBSENSE::saveCodebookParametersPath(std::string codebook_parameters_path) {
	variable_path = codebook_parameters_path;
}

void BackgroundSubtractorSuBSENSE::initialize(const cv::Mat& oInitImg, const cv::Mat& oROI) {

	progressFunctionDefined = false;
	...
	bool flg_load = false;
	if(loadvars){
		progressFunction("Preparing to load previous BGS model...", 0, 100);
		flg_load = loadVariables();
		progressFunction("Previous BGS model loaded...", 0, 100);
	}

	if(!flg_load|!loadvars)
	{
		progressFunction("Initializing color samples...", 0, 100);
			...
			//Label as background if at least "m_nRequiredBGSamples" matches were found
				...
				*pfCurrMeanRawSegmRes_ST = (*pfCurrMeanRawSegmRes_ST)*(1.0f-fRollAvgFactor_ST);
				...
				if((n_rand%(bCurrUsing3x3Spread?nLearningRate:(nLearningRate/2+1)))==0
					...
					const size_t s_rand = rand()%m_nBGSamples;
					int datsize = (int)(m_voBGDescSamples[s_rand].cols*m_voBGDescSamples[s_rand].rows*m_voBGDescSamples[s_rand].elemSize());
					for(size_t c=0; c<3; ++c) {
...
bool BackgroundSubtractorSuBSENSE::loadVariables() {	
	progressFunction("Reading model from file...", 0, 100);
	cv::FileStorage fs(variable_path, cv::FileStorage::READ);	
	progressFunction("Model read from file", 0, 100);
	
	fs["m_voBGColorSamples"] >> m_voBGColorSamples;
	progressFunction("Loaded m_voBGColorSamples...", 1, 8);

	fs["m_voBGDescSamples"] >> m_voBGDescSamples;
	progressFunction("Loaded m_voBGDescSamples...", 2, 8);

	fs["m_oUpdateRateFrame"] >> m_oUpdateRateFrame;
	progressFunction("Loaded m_oUpdateRateFrame...", 3, 8);

	fs["m_oDistThresholdFrame"] >> m_oDistThresholdFrame;	
	progressFunction("Loaded m_oDistThresholdFrame...", 4, 8);

	fs["m_oVariationModulatorFrame"] >> m_oVariationModulatorFrame;	
	progressFunction("Loaded m_oVariationModulatorFrame...", 5, 8);

	fs["m_oMeanLastDistFrame"] >> m_oMeanLastDistFrame;	
	progressFunction("Loaded m_oMeanLastDistFrame...", 6, 8);

	fs["m_oMeanMinDistFrame_LT"] >> m_oMeanMinDistFrame_LT;	
	progressFunction("Loaded m_oMeanMinDistFrame_LT...", 7, 8);

	fs["m_oMeanMinDistFrame_ST"] >> m_oMeanMinDistFrame_ST;
	progressFunction("Loaded m_oMeanMinDistFrame_ST...", 8, 8);
...
void BackgroundSubtractorSuBSENSE::saveVariables() {
	cv::FileStorage fs(variable_path, cv::FileStorage::WRITE);
...
void BackgroundSubtractorSuBSENSE::setBGSProgressFunction(bgsProgressCallbackFunction function) {
	if(function != NULL) {
		progressFunction = function;
		progressFunctionDefined = true;
	}	
}